...
import { GameEntity, GameEntityType } from '../../../hooks/useGameplayModals';
...
interface EntityDetailModalProps {
    ...
    onUpdateEntityAvatar: (entityType: 'npc' | 'yeuThu', entityId: string, newAvatarUrlOrData: string) => void; 
...
const AvatarEditor: React.FC<{
  ...
  onUpdateAvatar: (entityType: 'npc' | 'yeuThu', entityId: string, newAvatarUrlOrData: string) => void;
  isUploading: boolean;
}> = ({ entity, entityType, onUpdateAvatar, isUploading }) => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        onUpdateAvatar(entityType, entity.id, reader.result as string);
  ...
  const handleUrlSubmit = async () => {
    ...
    if (isValid) {
      onUpdateAvatar(entityType, entity.id, urlInput);
      setIsUrlInputVisible(false);
    } else {
      setUrlError(VIETNAMESE.avatarUrlInvalid);
    }
  };

  const handleRemove = () => {
      onUpdateAvatar(entityType, entity.id, '');
      setIsUrlInputVisible(false);
  };

  return (
    <div className="mt-4 pt-4 border-t border-gray-700 space-y-2">
        <h4 className="text-sm font-semibold text-gray-300 mb-2">Quản Lý Ảnh Đại Diện</h4>
        <div className="flex flex-col sm:flex-row gap-2">
             <Button size="sm" variant="secondary" className="w-full" onClick={() => fileInputRef.current?.click()} disabled={isUploading || isUrlValidating}>Tải Lên File</Button>
             <Button size="sm" variant="secondary" className="w-full" onClick={() => setIsUrlInputVisible(!isUrlInputVisible)} disabled={isUploading || isUrlValidating}>{isUrlInputVisible ? 'Đóng URL' : 'Nhập URL'}</Button>
             {entity.avatarUrl && <Button size="sm" variant="danger" className="w-full" onClick={handleRemove} disabled={isUploading || isUrlValidating}>Xóa Ảnh</Button>}
        </div>
        <input type="file" ref={fileInputRef} onChange={handleFileChange} accept="image/*" className="hidden" />

        {isUrlInputVisible && (
            <div className="p-2 border border-gray-600 rounded-md bg-gray-800/50 space-y-2">
                <InputField label="" id={`url-input-${entity.id}`} value={urlInput} onChange={e => setUrlInput(e.target.value)} placeholder="https://example.com/image.png" className="!mb-0"/>
                <Button size="sm" variant="primary" onClick={handleUrlSubmit} className="w-full" isLoading={isUrlValidating} disabled={isUploading || isUrlValidating}>Xác Nhận URL</Button>
                {urlError && <p className="text-xs text-red-400">{urlError}</p>}
            </div>
        )}
        {isUploading && <Spinner size="sm" text={VIETNAMESE.uploadingAvatarMessage} className="mt-2" />}
    </div>
  );
};
...
const EntityDetailModal: React.FC<EntityDetailModalProps> = ({ selectedEntity, isOpen, onClose, knowledgeBase, onUpdateEntityAvatar, isUploadingAvatar }) => {
    ...
    switch (type) {
            ...
            const renderStatBonuses = (bonuses: Partial<PlayerStats>) => {
                const relevantBonuses = Object.entries(bonuses).filter(
                    (entry): entry is [string, number] => {
                        ...
                        // This type predicate ensures only numeric, non-zero values are processed.
                ...
                return (
                    <ul className="list-disc list-inside pl-4 text-xs">
                        {relevantBonuses.map(([key, value]) => (
                            <li key={key}><span className="text-gray-300">{key}: </span><span className={value > 0 ? "text-green-400" : "text-red-400"}>{value > 0 ? `+${value}`: value}</span></li>
                        ))}
                    </ul>
                );
            };
        ...
        case 'slave': {
            ...
            const StatField = ({ label, value, valueClassName = "text-gray-200" }: { label: string; value: string | number | undefined; valueClassName?: string }) => (
                <div><p className="text-sm text-indigo-300">{label}</p><p className={`font-semibold ${valueClassName}`}>{value || 'Không rõ'}</p></div>
            );
            content = (
                <div className="space-y-4">
                    ...
                    <div className="bg-gray-900/40 p-3 rounded-lg"><h4 className="font-semibold text-lg text-indigo-300 border-b border-gray-600 pb-2 mb-3">Tu Luyện</h4><div className="grid grid-cols-2 gap-x-4 gap-y-3"><StatField label="Giới tính" value={person.gender} /><StatField label="Cảnh giới" value={person.realm} valueClassName="text-amber-400 font-bold"/><StatField label="Tư chất" value={person.tuChat} /><StatField label="Linh Căn" value={person.spiritualRoot} /><div className="col-span-2"><StatField label="Thể Chất" value={person.specialPhysique} /></div></div></div>
                    {('affinity' in person || 'willpower' in person || 'obedience' in person || 'resistance' in person) && (<div className="bg-gray-900/40 p-3 rounded-lg"><h4 className="font-semibold text-lg text-indigo-300 border-b border-gray-600 pb-2 mb-3">Trạng thái quan hệ</h4><div className="grid grid-cols-2 gap-x-4 gap-y-3"><StatField label="Thiện cảm" value={person.affinity} valueClassName={person.affinity >= 0 ? "text-green-400" : "text-red-400"}/>{('willpower' in person) && <StatField label="Ý chí" value={(person as any).willpower} />}{('obedience' in person) && <StatField label="Phục tùng" value={(person as any).obedience} />}{('resistance' in person) && <StatField label="Phản kháng" value={(person as Prisoner).resistance} />}</div></div>)}
                    {person.stats && (person.stats.maxSinhLuc != null || person.stats.sucTanCong != null) && (<div className="bg-gray-900/40 p-3 rounded-lg"><h4 className="font-semibold text-lg text-indigo-300 border-b border-gray-600 pb-2 mb-3">Chỉ số chiến đấu</h4><ul className="text-sm space-y-1.5">{person.stats.maxSinhLuc != null && <li><strong className="text-gray-400">Sinh Lực:</strong> {person.stats.sinhLuc ?? '??'} / {person.stats.maxSinhLuc}</li>}{person.stats.maxLinhLuc != null && person.stats.maxLinhLuc > 0 && <li><strong className="text-gray-400">Linh Lực:</strong> {person.stats.linhLuc ?? '??'} / {person.stats.maxLinhLuc}</li>}{person.stats.sucTanCong != null && <li><strong className="text-gray-400">Sức Tấn Công:</strong> {person.stats.sucTanCong}</li>}{person.stats.maxKinhNghiem != null && person.stats.maxKinhNghiem > 0 && <li><strong className="text-gray-400">Kinh Nghiệm:</strong> {person.stats.kinhNghiem ?? '??'} / {person.stats.maxKinhNghiem}</li>}{person.stats.thoNguyen !== undefined && <li className="col-span-2"><strong className="text-gray-400">Thọ Nguyên:</strong> {Math.floor(person.stats.thoNguyen)} / {person.stats.maxThoNguyen ?? '??'}</li>}</ul></div>)}
                    <p className="text-sm italic text-gray-400 pt-3 border-t border-gray-700/50">{person.description}</p>
                    {type === 'npc' && (
                        <AvatarEditor entity={person as NPC} entityType="npc" onUpdateAvatar={onUpdateEntityAvatar} isUploading={isUploadingAvatar} />
                    )}
                </div>
            );
            break;
        ...
        case 'yeuThu': {
            ...
            content = (
                <div className="space-y-2 text-sm">
                    ...
                    {yeuThu.stats && (<div className="mt-2 pt-2 border-t border-gray-700/50"><strong className="text-indigo-300">Chỉ số:</strong><ul className="list-disc list-inside pl-4 text-xs mt-1">{yeuThu.stats.maxSinhLuc != null && <li>Sinh Lực: {yeuThu.stats.sinhLuc ?? '??'} / {yeuThu.stats.maxSinhLuc}</li>}{yeuThu.stats.maxLinhLuc != null && yeuThu.stats.maxLinhLuc > 0 && <li>Linh Lực: {yeuThu.stats.linhLuc ?? '??'} / {yeuThu.stats.maxLinhLuc}</li>}{yeuThu.stats.sucTanCong != null && <li>Sức Tấn Công: {yeuThu.stats.sucTanCong}</li>}</ul></div>)}
                     <AvatarEditor entity={yeuThu} entityType="yeuThu" onUpdateAvatar={onUpdateEntityAvatar} isUploading={isUploadingAvatar} />
                </div>
            );
            break;
        }
        case 'location': {
            ...
            const getRegionName = (regionId?: string): string | undefined => { if (!regionId) return undefined; return knowledgeBase.discoveredRegions.find(r => r.id === regionId)?.name; };
            ...
            content = (<div className="space-y-2 text-sm"><p><strong className="text-indigo-300">Tên:</strong> {location.name}</p>{location.locationType && <p><strong className="text-indigo-300">Loại:</strong> {location.locationType}</p>}{(location.mapX !== undefined && location.mapY !== undefined) && (<p><strong className="text-indigo-300">Tọa độ:</strong> ({location.mapX}, {location.mapY})</p>)}{regionName && <p><strong className="text-indigo-300">{VIETNAMESE.regionLabel || "Vùng"}:</strong> {regionName}</p>}<p><strong className="text-indigo-300">An toàn:</strong> {location.isSafeZone ? "Có" : "Không"}</p>{location.parentLocationId === undefined && (<p><strong className="text-indigo-300">{VIETNAMESE.locationStatusLabel}:</strong> <span className={location.visited ? 'text-green-400 font-semibold' : 'text-gray-400'}>{location.visited ? VIETNAMESE.locationStatusVisited : VIETNAMESE.locationStatusUnvisited}</span></p>)}{location.connections && location.connections.some(c => c.isDiscovered) && (<div className="mt-2"><strong className="text-indigo-300">Kết nối đã biết:</strong><ul className="list-disc list-inside pl-4 text-gray-300 text-xs">{location.connections.filter(c => c.isDiscovered).map(conn => { const target = knowledgeBase.discoveredLocations.find(l => l.id === conn.targetLocationId); let connDetails = target?.name || conn.targetLocationId; if (conn.travelTimeTurns) connDetails += ` (Thời gian: ${conn.travelTimeTurns} lượt)`; if (conn.description) connDetails += ` - ${conn.description}`; return <li key={conn.targetLocationId}>{connDetails}</li>; })}</ul></div>)}{subLocations.length > 0 && (<div className="mt-2"><strong className="text-indigo-300">Khu vực phụ:</strong><ul className="list-disc list-inside pl-4 text-gray-300 text-xs">{subLocations.map(subLoc => <li key={subLoc.id}>{subLoc.name}</li>)}</ul></div>)}</div>);
            break;
        }
        case 'lore': content = <p className="whitespace-pre-wrap"><strong className="text-indigo-300">Nội dung:</strong> {(entity as WorldLoreEntry).content}</p>; break;
        case 'companion': content = <><p><strong className="text-indigo-300">HP:</strong> {(entity as Companion).hp}/{(entity as Companion).maxHp}</p><p><strong className="text-indigo-300">ATK:</strong> {(entity as Companion).atk}</p><p><strong className="text-indigo-300">Mô tả:</strong> {(entity as Companion).description}</p></>; break;
        case 'faction': content = <div className="space-y-2"><p><strong className="text-indigo-300">Tên Phe Phái:</strong> {(entity as Faction).name}</p>{(entity as Faction).description && <p><strong className="text-indigo-300">Mô tả:</strong> {(entity as Faction).description}</p>}<p><strong className="text-indigo-300">Chính tà:</strong> {(entity as Faction).alignment}</p><p><strong className="text-indigo-300">Uy tín người chơi:</strong> {String((entity as Faction).playerReputation)}</p></div>; break;
    }
