...
import { useGameplayModals, GameEntity, GameEntityType } from '../hooks/useGameplayModals'; // Import modal hook and types
import { calculateRealmBaseStats, calculateEffectiveStats, getMessagesForPage, performTagProcessing, handleLevelUps, progressNpcCultivation } from '../utils/gameLogicUtils'; 
import { summarizeTurnHistory, getApiSettings as getGeminiApiSettings, countTokens, generateCraftedItemViaAI, generateDefeatConsequence } from '../services/geminiService'; 
...
export const GameProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const { 
        selectedEntity, isStyleSettingsModalOpen, activeEconomyModal, activeSlaveMarketModal,
        openEntityModal, closeModal, closeEconomyModal, closeSlaveMarketModal,
        setIsStyleSettingsModalOpen, setActiveEconomyModal, setActiveSlaveMarketModal
    } = useGameplayModals();
    ...
    const onUpdateEntityAvatar = useCallback(async (entityType: 'npc' | 'yeuThu', entityId: string, newAvatarUrlOrData: string) => {
        setIsUploadingAvatar(true);
        let finalUrlForEntity = newAvatarUrlOrData;
        
        let entityData: NPC | YeuThu | undefined;
        if (entityType === 'npc') {
            entityData = knowledgeBase.discoveredNPCs.find(n => n.id === entityId);
        } else {
            entityData = knowledgeBase.discoveredYeuThu.find(y => y.id === entityId);
        }

        if (!entityData) {
            showNotification(`Không tìm thấy thực thể để cập nhật avatar.`, "error");
            setIsUploadingAvatar(false);
            return;
        }
        ...
        if (newAvatarUrlOrData.startsWith('data:image')) {
            try {
                const base64StringOnly = newAvatarUrlOrData.split(',')[1];
                let cloudinaryFolderType: 'npc_male' | 'npc_female' | 'yeu_thu';
                if (entityType === 'npc') {
                    cloudinaryFolderType = (entityData as NPC).gender === 'Nữ' ? 'npc_female' : 'npc_male';
                } else {
                    cloudinaryFolderType = 'yeu_thu';
                }
                finalUrlForEntity = await uploadImageToCloudinary(base64StringOnly, cloudinaryFolderType, `${entityType}_${entityId}_ingame`);
            } catch (uploadError) {
                console.error(`Cloudinary upload failed for ${entityType} ${entityId}:`, uploadError);
                showNotification(VIETNAMESE.avatarUploadError + (uploadError instanceof Error ? uploadError.message : ""), "error");
                setIsUploadingAvatar(false);
                return;
            }
            ...
            finalUrlForEntity = newAvatarUrlOrData;
        } else if (newAvatarUrlOrData === '') {
            finalUrlForEntity = ''; 
        } else {
            showNotification("Định dạng avatar không hợp lệ.", "error");
        ...
        setKnowledgeBase(prevKb => {
            const newKb = JSON.parse(JSON.stringify(prevKb));
            const listToUpdate: (NPC[] | YeuThu[]) = entityType === 'npc' ? newKb.discoveredNPCs : newKb.discoveredYeuThu;
            const entityIndex = listToUpdate.findIndex((e: NPC | YeuThu) => e.id === entityId);
            if (entityIndex > -1) {
                listToUpdate[entityIndex].avatarUrl = finalUrlForEntity === '' ? undefined : finalUrlForEntity;
            }
            return newKb;
        });

        showNotification(`${VIETNAMESE.avatarUploadSuccess} cho ${entityData.name}.`, 'success');
        setIsUploadingAvatar(false);
    }, [knowledgeBase, setKnowledgeBase, showNotification]);
    ...
    const onRollbackTurn = useCallback(() => {
        ...
        if (stateToRestore && stateToRestore.knowledgeBaseSnapshot) {
            setKnowledgeBase({ ...stateToRestore.knowledgeBaseSnapshot, turnHistory: historyCopy });
    ...
    const convertPrisoner = useCallback((prisonerId: string, targetType: 'wife' | 'slave') => {
        setKnowledgeBase(prevKb => {
            ...
            if (targetType === 'wife') {
                const newWife: Wife = { ...baseData, entityType: 'wife', skills: [], equippedItems: { mainWeapon: null, offHandWeapon: null, head: null, body: null, hands: null, legs: null, artifact: null, pet: null, accessory1: null, accessory2: null }};
                newKb.wives.push(newWife);
            } else { 
                const newSlave: Slave = { ...baseData, entityType: 'slave', skills: [], equippedItems: { mainWeapon: null, offHandWeapon: null, head: null, body: null, hands: null, legs: null, artifact: null, pet: null, accessory1: null, accessory2: null }};
    ...
    const renameSlave = useCallback((slaveId: string, newName: string) => {
        ...
        if (!trimmedName) { showNotification("Tên không được để trống.", "error"); return; }
        const slaveToRename = knowledgeBase.slaves.find(s => s.id === slaveId);
        if (!slaveToRename) { showNotification("Không tìm thấy nô lệ.", "error"); return; }
        const allNames = [knowledgeBase.worldConfig?.playerName, ...knowledgeBase.discoveredNPCs.map(n => n.name), ...knowledgeBase.wives.map(w => w.name), ...knowledgeBase.slaves.filter(s => s.id !== slaveId).map(s => s.name), ...knowledgeBase.prisoners.map(p => p.name)].filter(Boolean).map(n => (n || '').toLowerCase());
        if (allNames.includes(trimmedName.toLowerCase())) { showNotification(`Tên "${trimmedName}" đã tồn tại. Vui lòng chọn tên khác.`, "error"); return; }
    ...
    const contextValue: GameContextType = {
        currentScreen, knowledgeBase, gameMessages, styleSettings, storageSettings, isInitialLoading, storageInitError, notification, apiError, isLoadingApi, isSummarizingNextPageTransition, isAutoPlaying, isSavingGame, isAutoSaving, isSummarizingOnLoad, isCraftingItem, isUploadingAvatar, isCultivating,
        sentCultivationPromptsLog: gameData.sentCultivationPromptsLog, receivedCultivationResponsesLog: gameData.receivedCultivationResponsesLog,
        prisonerInteractionLog: gameData.prisonerInteractionLog, companionInteractionLog: gameData.companionInteractionLog,
        sentPrisonerPromptsLog: gameData.sentPrisonerPromptsLog, receivedPrisonerResponsesLog: gameData.receivedPrisonerResponsesLog,
        sentCompanionPromptsLog: gameData.sentCompanionPromptsLog, receivedCompanionResponsesLog: gameData.receivedCompanionResponsesLog,
        rawAiResponsesLog: gameData.rawAiResponsesLog, sentPromptsLog: gameData.sentPromptsLog, sentEconomyPromptsLog: gameData.sentEconomyPromptsLog, receivedEconomyResponsesLog: gameData.receivedEconomyResponsesLog,
        sentGeneralSubLocationPromptsLog: gameData.sentGeneralSubLocationPromptsLog, receivedGeneralSubLocationResponsesLog: gameData.receivedGeneralSubLocationResponsesLog,
        retrievedRagContextLog: gameData.retrievedRagContextLog, latestPromptTokenCount: gameData.latestPromptTokenCount, summarizationResponsesLog: gameData.summarizationResponsesLog,
        sentCraftingPromptsLog: gameData.sentCraftingPromptsLog, receivedCraftingResponsesLog: gameData.receivedCraftingResponsesLog, sentNpcAvatarPromptsLog,
        sentCombatSummaryPromptsLog: gameData.sentCombatSummaryPromptsLog, receivedCombatSummaryResponsesLog: gameData.receivedCombatSummaryResponsesLog,
        sentVictoryConsequencePromptsLog: gameData.sentVictoryConsequencePromptsLog, receivedVictoryConsequenceResponsesLog: gameData.receivedVictoryConsequenceResponsesLog,
        currentPageDisplay, totalPages, messageIdBeingEdited, currentPageMessagesLog, previousPageSummaries: previousPageSummariesContent, lastNarrationFromPreviousPage,
        selectedEntity, isStyleSettingsModalOpen, activeEconomyModal, activeSlaveMarketModal,
        openEntityModal, closeModal, closeEconomyModal, closeSlaveMarketModal, setIsStyleSettingsModalOpen, setActiveEconomyModal, setActiveSlaveMarketModal,
        setCurrentScreen, setKnowledgeBase, setGameMessages, setStyleSettings, setRetrievedRagContextLog,
        handleSetupComplete: handleSetupCompleteWrapper, handlePlayerAction: gameActions.handlePlayerAction, handleFindLocation: gameActions.handleFindLocation,
        handleNonCombatDefeat: gameActions.handleNonCombatDefeat, handlePrisonerAction: gameActions.handlePrisonerAction, handleCompanionAction: gameActions.handleCompanionAction,
        handleExitPrisonerScreen: gameActions.handleExitPrisonerScreen, handleExitCompanionScreen: gameActions.handleExitCompanionScreen,
        onQuit, onSaveGame, onLoadGame, handleUpdateEquipment, handleCraftItem, handleBuyItem, handleSellItem, handleCombatEnd,
        onGoToNextPage: () => setCurrentPageDisplay(prev => Math.min(prev + 1, totalPages)), onGoToPrevPage: () => setCurrentPageDisplay(prev => Math.max(prev - 1, 1)), onJumpToPage: (page: number) => setCurrentPageDisplay(Math.max(1, Math.min(page, totalPages))),
        getMessagesForPage: getMessagesForPageFromHook, isCurrentlyActivePage: currentPageDisplay === totalPages,
        onRollbackTurn, onToggleAutoPlay: () => setIsAutoPlaying(prev => !prev), onStartEditMessage, onSaveEditedMessage, onCancelEditMessage,
        onUpdatePlayerAvatar: handleUpdatePlayerAvatar, onUpdateEntityAvatar, handleStartAuction: gameActions.handleStartAuction,
        handlePlayerAuctionAction: gameActions.handlePlayerAuctionAction, handleAuctioneerCall: gameActions.handleAuctioneerCall,
        handleSkipAuctionItem: gameActions.handleSkipAuctionItem, handleStartCultivation: gameActions.handleStartCultivation,
        handleExitCultivation: gameActions.handleExitCultivation, fetchSaveGamesForImportExport, loadSpecificGameDataForExport,
        handleImportGameData, showNotification, onSettingsSaved, gameplayScrollPosition, justLoadedGame, convertPrisoner,
        handleProcessDebugTags, handleBuySlave: gameActions.handleBuySlave, handleSellSlave: gameActions.handleSellSlave,
        handleStartSlaveAuction: gameActions.handleStartSlaveAuction, handlePlayerSlaveAuctionAction: gameActions.handlePlayerSlaveAuctionAction,
        handleSlaveAuctioneerCall: gameActions.handleSlaveAuctioneerCall, handleSkipSlaveAuctionItem: gameActions.handleSkipSlaveAuctionItem,
        renameSlave, updateLocationCoordinates,
    };
